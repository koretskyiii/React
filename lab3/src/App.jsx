    ...
    <div className='flex flex-col justify-center items-center mt-5'>
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col w-1/3 p-5 gap-2 border-black border'>
        <div className='flex flex-col'>
          ...
          <input {...register('email')} className='input border border-black w-72' />
          {errors.email && <span className='text-red-600'>{errors.email.message}</span>}
        </div>
        <div className='flex flex-col'>
          <label htmlFor="name">Вкажіть ім'я</label>
          <input {...register('name', { required: true })} className='border border-black w-72' />
        ...
        <div>
          {sizes.map((size, index) => (
            <div key={index} className='w-72'>
              <input type="radio" name="size" value={size} {...register('size')} className='border border-black' />
        ...
        <input type='submit' className='border border-black bg-slate-200 cursor-pointer' />
      </form>
    </div>
  );
}
